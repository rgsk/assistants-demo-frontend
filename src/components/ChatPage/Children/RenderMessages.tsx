  ...
  return html`<div class="whitespace-normal rounded-lg overflow-hidden mb-4">
    ...
    >
      ...
      <button class="code-copy">
        <span class="flex items-center gap-1"> ${copyCodeHtml}</span>
  ...
  useEffect(() => {
    ...
    for (const button of buttons) {
      button.addEventListener("click", () => {
        const code = button.children[1].textContent;
        navigator.clipboard.writeText(code!);
        button.children[0].innerHTML = copiedHtml;
        setTimeout(() => {
          button.children[0].innerHTML = copyCodeHtml;
        }, 2000);
      });
    }
  }, [messages]);
      ...
      {messages?.map((item, i) => (
        <div key={i}>
          <div className="flex items-start gap-2">
            {item.role == "user" ? (
              <div className="bg-[#7988FF] rounded-full p-1">
                <AvatarSVG />
              </div>
            ) : (
